...
    activeRideErrorTitle: "Cannot Post New Ride",
    youHaveActiveRide: "You have an active ride. Finish it first to post a new one.",
    finishRide: "Finish Ride",
    finish: "Finish",
    finishRideConfirmation: "Are you sure you want to finish this ride?",
    history: "History",
  },
  uz: {
...
    activeRideErrorTitle: "Yangi Sayohat Joylab Bo'lmaydi",
    youHaveActiveRide: "Sizda faol sayohat mavjud. Yangisini joylash uchun avval joriyni yakunlang.",
    finishRide: "Sayohatni yakunlash",
    finish: "Yakunlash",
    finishRideConfirmation: "Ushbu sayohatni yakunlashga ishonchingiz komilmi?",
    history: "Tarix",
  },
  ru: {
...
    activeRideErrorTitle: "Невозможно Опубликовать Новую Поездку",
    youHaveActiveRide: "У вас есть активная поездка. Сначала завершите ее, чтобы опубликовать новую.",
    finishRide: "Завершить поездку",
    finish: "Завершить",
    finishRideConfirmation: "Вы уверены, что хотите завершить эту поездку?",
    history: "История",
  },
};
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 font-sans">
            <div className="bg-white rounded-3xl shadow-lg w-full max-w-sm p-6 text-center">
                ...
                <div className="flex justify-center">
                    <button onClick={onClose} className="w-full py-3 px-4 bg-green-500 text-white font-semibold rounded-xl hover:bg-green-600 transition-colors">
                        {t('okay')}
                    </button>
                </div>
            </div>
...
const uzbekistanCars = [
    // Chevrolet
    'Chevrolet Cobalt', 'Chevrolet Lacetti (Gentra)', 'Chevrolet Nexia 3', 'Chevrolet Spark', 
    'Chevrolet Damas', 'Chevrolet Onix', 'Chevrolet Tracker', 'Chevrolet Malibu', 
    'Chevrolet Equinox', 'Chevrolet Captiva',
    // Daewoo (Still common)
    'Daewoo Matiz', 'Daewoo Nexia', 'Daewoo Tico',
    // BYD (Popular EV/Hybrids)
    'BYD Song Plus', 'BYD Chazor (Seal)', 'BYD Han', 'BYD Tang',
    // Chery
    'Chery Tiggo 7 Pro', 'Chery Tiggo 8 Pro', 'Chery Arrizo 6 Pro',
    // Kia
    'Kia K5', 'Kia Cerato', 'Kia Seltos', 'Kia Sportage', 'Kia Sorento', 'Kia Carnival',
    // Hyundai
    'Hyundai Elantra', 'Hyundai Sonata', 'Hyundai Tucson', 'Hyundai Santa Fe', 'Hyundai Creta', 'Hyundai Accent',
    // Lada
    'Lada Vesta', 'Lada Granta', 'Lada Niva Legend',
    // Other common models
    'Toyota Camry', 'Volkswagen Jetta', 'Skoda Kodiaq'
];
...
// --- Ride History Components (moved from ProfilePage for reusability) ---
const RideHistoryContent = ({ myRides }) => {
    const { t } = useLanguage();
    const completedRides = myRides.filter(r => r.status === 'completed');
    return (
        <div>
            <h3 className="font-semibold mb-2 text-gray-800">{t('pastRides')}</h3>
            {completedRides.length > 0 ? completedRides.map(ride => (
                <div key={ride.id} className="mb-2 p-3 bg-neutral-100/50 rounded-lg">
                    <p className="font-medium">{ride.departure_location} to {ride.destination_location}</p>
                    <p className="text-xs text-neutral-500">{ride.departure_date}</p>
                </div>
...
const UpcomingRidesContent = ({ myRides }) => {
    const { t } = useLanguage();
    const upcomingRides = myRides.filter(r => r.status === 'upcoming' || r.status === 'active');
    return (
        <div>
            <h3 className="font-semibold mb-2 text-gray-800">{t('upcomingRides')}</h3>
...
const HistoryPage = ({ myRides }) => {
    ...
    return (
        <div className="p-4 space-y-4 text-gray-800 font-sans pb-20">
            <div className="bg-white/80 backdrop-blur-sm p-4 rounded-2xl shadow-lg border border-white/20 space-y-3">
                {sortedRides.length > 0 ? sortedRides.map(ride => (
                    <div key={ride.id} className="p-3 bg-neutral-100/50 rounded-lg flex justify-between items-center">
                        <div>
                            <p className="font-medium">{ride.departure_location} to {ride.destination_location}</p>
                            <p className="text-xs text-neutral-500">{ride.departure_date}</p>
                        </div>
                    ...
                    <p className="text-neutral-500 text-center py-4">{t('noCompletedRides')}</p>
                )}
            </div>
        </div>
    );
...
      if (typeof window !== 'undefined' && !localStorage.getItem('username')) {
        localStorage.setItem('username', 'demo-user-' + Math.random().toString(36).substring(7));
      };
      
...
  };
  ...
  const executeFinishRide = () => {
    ...
    toast({
        title: "Ride Finished!",
        description: "Your earnings will be updated shortly."
    });
  };
...
                  <div className="border-t border-neutral-200/50 p-4">
                      {isRideStarted ? (
                          <button onClick={handleFinishRide} className="w-full py-3 px-4 bg-red-500 text-white font-semibold rounded-xl hover:bg-red-600 transition-colors flex items-center justify-center">
...
          </div>
          <div>
              <div className="w-full flex justify-between items-center p-1 group">
              	<h3 className="flex items-center text-sm font-semibold text-black drop-shadow-[0_1px_1px_rgba(255,255,255,0.7)]">
...
        </div>
      );
      case "history": return ( <HistoryPage myRides={myRides} /> );
...
       )}
      <InfoModal
        isOpen={showActiveRideErrorModal}
        onClose={() => setShowActiveRideErrorModal(false)}
        title={t('activeRideErrorTitle')}
        message={t('youHaveActiveRide')}
      />
      {showFinishRideModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 font-sans">
            <div className="bg-white rounded-3xl shadow-lg w-full max-w-sm p-6 text-center">
                <h2 className="text-xl font-bold text-gray-800 mb-2">{t('finishRide')}</h2>
                <p className="text-neutral-600 mb-6">{t('finishRideConfirmation')}</p>
                <div className="flex justify-center gap-4">
                    <button onClick={() => setShowFinishRideModal(false)} className="flex-1 py-2 px-4 bg-neutral-200 text-neutral-800 font-semibold rounded-xl hover:bg-neutral-300 transition-colors">
                        {t('cancel')}
                    </button>
                    <button onClick={executeFinishRide} className="flex-1 py-2 px-4 bg-red-500 text-white font-semibold rounded-xl hover:bg-red-600 transition-colors">
                        {t('finish')}
                    </button>
                </div>
            </div>
...
    const recentTrips = [
        {id: 1, from: 'Tashkent', to: 'Samarkand', earnings: 50},
        {id: 2, from: 'Bukhara', to: 'Khiva', earnings: 70},
