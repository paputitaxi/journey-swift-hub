...
const CustomScrollbarStyles = () => (
  <style>{`
    ...
    .custom-scrollbar::-webkit-scrollbar-track { background: #f4f1de; }
    .custom-scrollbar::-webkit-scrollbar-thumb { background-color: #a19b87; border: 2px solid #f4f1de; }
    .custom-scrollbar::-webkit-scrollbar-thumb:hover { background-color: #4a2c2a; }
  `}</style>
);
...
const PlateNumber = ({ plate }) => {
  ...
  return (
    <div className="flex items-center bg-[#f4f1de] border-2 border-[#4a2c2a] p-1 space-x-2 w-max select-none">
      <span className="font-bold text-xl pl-1">{plate.locationCode}</span>
      <div className="w-px h-6 bg-[#4a2c2a]"></div>
      ...
      <div className="flex flex-col items-center pr-1">
        <svg width="28" height="14" viewBox="0 0 900 450" className="border border-[#a19b87]">
        ...
        <span className="font-bold text-xs text-[#4a2c2a]">UZ</span>
      </div>
    </div>
  );
};
...
const AmazonMailLogo = ({ className }) => (
  <svg className={className} width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M4 7.00005L10.2 10.1C11.2667 10.6333 12.7333 10.6333 13.8 10.1L20 7"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
    ...
    <path
      d="M7 14.5C8.5 13 11 13 12 14.5C13 13 15.5 13 17 14.5"
      stroke="currentColor"
      strokeWidth="1.5"
      strokeLinecap="round"
    />
...
const TipBar = ({ icon, text, onClose }) => {
  return (
    <div className="bg-[#fffbeb] border-l-4 border-[#d9573c] p-4 flex items-center justify-between text-[#4a2c2a]">
      <div className="flex items-center">
        <div className="bg-[#d9573c] rounded-full p-2 mr-4 text-white">{icon}</div>
        <p className="text-sm">{text}</p>
      </div>
      <button onClick={onClose} className="p-1 rounded-full hover:bg-[#a19b8744]">
        <X size={20} />
      </button>
    </div>
  );
...
const CalendarView = ({ onDayClick, selectedDate }) => {
  ...
  return (
    <div className="bg-[#fffbeb] p-4">
      ...
      <div className="grid grid-cols-7 gap-2 text-center text-sm text-[#a19b87] mb-2">
        ...
        {days.map((day) => {
          ...
          return (
            <button
              ...
              disabled={isPast}
              className={`w-10 h-10 flex items-center justify-center border-2 border-[#4a2c2a] transition-colors ${
                isPast
                  ? "text-[#a19b87] cursor-not-allowed"
                  : isSelected
                    ? "bg-[#4a2c2a] text-[#f4f1de] font-semibold"
                    : "hover:bg-[#a19b8744]"
              }`}
            >
              {day}
...
const CalendarModal = ({ isOpen, onClose, onDayClick, selectedDate }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[#fffbeb] border-4 border-[#4a2c2a] shadow-[8px_8px_0px_#4a2c2a] w-full max-w-sm">
        <div className="p-4 border-b-2 border-[#4a2c2a] flex justify-between items-center">
          <h2 className="font-semibold text-lg">Select Date</h2>
          <button onClick={onClose} className="p-1 border-2 border-[#4a2c2a] hover:bg-[#a19b8744]">
            <X size={20} />
          </button>
        </div>
...
const LocationSelectModal = ({ title, isOpen, onClose, onSelect }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 font-sans">
      <div className="bg-[#fffbeb] border-4 border-[#4a2c2a] shadow-[8px_8px_0px_#4a2c2a] w-full max-w-md h-[80vh] flex flex-col">
        <div className="p-4 border-b-2 border-[#4a2c2a] flex justify-between items-center">
          <h2 className="text-lg font-semibold">{title}</h2>
          <button onClick={handleClose} className="p-1 border-2 border-[#4a2c2a] hover:bg-[#a19b8744]">
            <X className="h-6 w-6" />
          </button>
        </div>
        <div className="p-4 border-b-2 border-[#4a2c2a]">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#a19b87] h-5 w-5" />
            <input
              type="text"
              placeholder="Search..."
              className="w-full pl-10 pr-4 py-2 bg-[#f4f1de] border-2 border-[#4a2c2a] focus:outline-none focus:bg-yellow-50"
              value={searchTerm}
          ...
          {filteredLocations.length > 0 ? (
            filteredLocations.map((regionData) => (
              <div key={regionData.region}>
                <h3 className="w-full text-left p-3 font-semibold select-none">{regionData.region}</h3>
                {regionData.cities.map((city) => (
                  <button
                    ...
                    className="w-full text-left pl-6 py-2 hover:bg-[#a19b8744] text-sm"
                  >
                    {city}
                  </button>
                ))}
            ...
            <p className="text-[#a19b87] text-center mt-10">No locations found.</p>
          )}
        </div>
      </div>
    </div>
...
const BookingModal = ({ isOpen, onClose, ride, onConfirmBooking }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 font-sans">
      <div className="bg-[#fffbeb] border-4 border-[#4a2c2a] shadow-[8px_8px_0px_#4a2c2a] w-full max-w-md flex flex-col">
        <div className="p-4 border-b-2 border-[#4a2c2a] flex justify-between items-center">
        ...
        <div className="p-6 space-y-4">
          <div>
            <p className="text-sm text-[#a19b87]">Booking ride from</p>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <button
                ...
                className="p-2 border-2 border-[#4a2c2a] hover:bg-[#a19b8744] disabled:opacity-50"
                disabled={seats <= 1}
              >
                <Minus size={16} />
              </button>
          ...
          <div className="flex justify-between items-baseline pt-4 border-t-2 border-[#4a2c2a]">
            <span className="font-semibold text-lg">Total:</span>
            <span className="font-bold text-2xl text-[#1b998b]">${totalPrice}</span>
          </div>
        </div>
        <div className="p-4 bg-[#f4f1de]">
          <button
            onClick={handleConfirm}
            className="w-full bg-[#d9573c] text-[#f4f1de] py-3 border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all"
          >
            CONFIRM
          </button>
        </div>
...
const LanguageSelectModal = ({ isOpen, onClose, onSelect, currentLanguage, t }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 font-sans">
      <div className="bg-[#fffbeb] border-4 border-[#4a2c2a] shadow-[8px_8px_0px_#4a2c2a] w-full max-w-sm flex flex-col">
        <div className="p-4 border-b-2 border-[#4a2c2a] flex justify-between items-center">
        ...
        <div className="p-2">
          {languages.map((lang) => (
            <button
              ...
              className={`w-full text-left p-4 flex justify-between items-center hover:bg-[#a19b8744] ${currentLanguage === lang.code ? "font-semibold text-[#1b998b]" : ""}`}
            >
              <span>{lang.name}</span>
              {currentLanguage === lang.code && <div className="w-3 h-3 bg-[#1b998b] border-2 border-[#4a2c2a]"></div>}
            </button>
          ))}
        </div>
      </div>
...
const App = () => {
  ...
  const handleSaveRide = (rideId) => {
    setSavedRides((prevSaved) =>
      prevSaved.includes(rideId) ? prevSaved.filter((id) => id !== rideId) : [...prevSaved, rideId],
    );
  };
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "history":
        return (
          <div className="p-6">
            <h2 className="text-2xl font-semibold mb-4">{t.rideHistory}</h2>
            {rideHistory.length > 0 ? (
              <div className="space-y-4">
                {rideHistory.map((ride) => (
                  <div
                    key={`${ride.id}-${ride.bookingDate}`}
                    className="bg-[#fffbeb] p-4 border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a]"
                  >
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <p className="text-sm text-[#a19b87]">
                          {t.bookedOn} {formatDate(ride.bookingDate)}
                        </p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-lg text-[#1b998b]">
              ...
              <p className="text-[#a19b87] text-center mt-10">{t.pastRidesAppearHere}</p>
            )}
          </div>
        );
      case "search":
        ...
        if (showSearchResults) {
          ...
          const tipContent = {
            by_time: {
              ...
              text: "The ride will start at the scheduled time, regardless of seat availability.",
            },
            by_seat: { icon: <Users size={20} />, text: "The driver departs once all seats are filled." },
            saved: { icon: <Bookmark size={20} />, text: "Showing only the rides you have saved." },
            recommended: {
              icon: <Lightbulb size={20} />,
              text: "Showing recommended rides based on driver ratings and your history.",
            },
            mail: {
              icon: <AmazonMailLogo />,
              text: "Showing rides that can also deliver mail. Mail should not be more than 5 kg.",
            },
          };
          ...
          return (
            <div className="flex flex-col h-full">
              <div className="flex-shrink-0 bg-[#fffbeb] z-10 border-b-2 border-[#4a2c2a]">
                <div className="p-4">
                  <div className="flex items-center mb-3">
                    <button
                      className="p-2 border-2 border-[#4a2c2a] hover:bg-[#a19b8744] -ml-2 mr-2"
                      onClick={() => {
                        setPickupLocation("");
                    ...
                    <h2 className="text-lg font-semibold text-left">{t.results}</h2>
                  </div>
                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                      <div className="flex items-center space-x-2 flex-wrap gap-2">
                        <button
                          ...
                          className={`flex items-center space-x-2 px-3 py-2 text-sm font-semibold transition-colors border-2 border-[#4a2c2a] ${activeSort === "by_seat" ? "bg-[#4a2c2a] text-[#f4f1de]" : "hover:bg-[#a19b8744]"}`}
                        >
                          <Users size={16} />
                          <span>{t.bySeat}</span>
                        </button>
                        <button
                          ...
                          className={`flex items-center space-x-2 px-3 py-2 text-sm font-semibold transition-colors border-2 border-[#4a2c2a] ${activeSort === "by_time" ? "bg-[#4a2c2a] text-[#f4f1de]" : "hover:bg-[#a19b8744]"}`}
                        >
                          <History size={16} />
                          <span>{t.byTime}</span>
                        </button>
                        <button
                          ...
                          className={`flex items-center space-x-2 px-3 py-2 text-sm font-semibold transition-colors border-2 border-[#4a2c2a] ${activeFilter === "mail" ? "bg-[#4a2c2a] text-[#f4f1de]" : "hover:bg-[#a19b8744]"}`}
                        >
                      ...
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          className={`flex items-center space-x-2 px-3 py-2 text-sm font-semibold transition-colors border-2 border-[#4a2c2a] ${activeFilter === "saved" ? "bg-[#4a2c2a] text-[#f4f1de]" : "hover:bg-[#a19b8744]"}`}
                        >
                        ...
                        <button
                          ...
                          className={`flex items-center space-x-2 px-3 py-2 text-sm font-semibold transition-colors border-2 border-[#4a2c2a] ${activeFilter === "recommended" ? "bg-[#4a2c2a] text-[#f4f1de]" : "hover:bg-[#a19b8744]"}`}
                        >
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-sm font-semibold">{t.seatsNeeded}</span>
                      {[1, 2, 3, 4].map((num) => (
                        <button
                          key={num}
                          onClick={() => handleSeatsNeededClick(num)}
                          className={`w-10 h-10 text-sm font-semibold transition-colors border-2 border-[#4a2c2a] flex items-center justify-center ${seatsNeeded === num ? "bg-[#d9573c] text-white" : "hover:bg-[#a19b8744]"}`}
                        >
                          {num}
                        </button>
                      ))}
                ...
                {tipToShow && (
                  <TipBar
                    icon={tipContent[tipToShow].icon}
                    text={tipContent[tipToShow].text}
                    onClose={handleCloseTip}
                  />
                )}

                <div className="bg-[#fffbeb] py-2 px-4 border-t-2 border-[#4a2c2a]">
                  <h3 className="font-semibold">{stickyTitle}</h3>
                </div>
              </div>

              <div className="flex-grow overflow-y-auto bg-[#f4f1de]">
                <div className="p-4 space-y-4">
                  {results.map((item) => (
                    <div
                      key={item.id}
                      onClick={() => handleCardClick(item)}
                      className="bg-[#fffbeb] p-4 border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a] text-left cursor-pointer hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all"
                    >
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center">
                          ...
                          />
                          <p className="font-semibold text-lg">{item.driverName}</p>
                        </div>

                        {/* Route */}
                        <div className="flex items-center space-x-3">
                          <div className="flex items-center space-x-2 flex-1">
                            <MapPin size={16} className="text-[#1b998b]" />
                            <span className="font-medium">{item.origin}</span>
                          </div>
                          <div className="text-[#a19b87]">→</div>
                          <div className="flex items-center space-x-2 flex-1">
                            <Target size={16} className="text-[#d9573c]" />
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <Calendar size={16} />
                            <span>{formatDate(item.originDate)}</span>
                            <span>at {formatTime(item.originDate)}</span>
                          </div>
                        </div>

                        {/* Seats and Price */}
                        <div className="flex items-center justify-between text-sm pt-2 border-t-2 border-[#a19b87] mt-2">
                          <div className="flex items-center space-x-4">
                            {(item.serviceType === "mail" || item.serviceType === "both") && (
                              <div className="flex items-center space-x-1 text-[#1b998b]">
                                <Mail size={16} />
                          ...
                          <div className="text-xl font-bold text-[#1b998b]">${item.basePrice.toFixed(2)}</div>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          return (
            <div className="p-6 text-center">
              <h2 className="text-2xl font-semibold mb-2">{t.findYourNextRide}</h2>
              <p className="text-md mb-6">{t.selectPickupAndDestination}</p>
              <div className="bg-[#fffbeb] p-6 border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a] space-y-4">
                <div
                  onClick={() => setShowFromModal(true)}
                  className="w-full p-3 bg-[#f4f1de] border-2 border-[#4a2c2a] flex items-center justify-between cursor-pointer hover:bg-[#a19b8744]"
                >
                ...
                <div className="pt-4 border-t-2 border-[#4a2c2a]">
                  <p className="text-left font-semibold mb-2">{t.howManySeats}</p>
                  <div className="flex items-center justify-center space-x-4">
                    {[1, 2, 3, 4].map((num) => (
                      <button
                        key={num}
                        onClick={() => handleSeatsNeededClick(num)}
                        className={`w-12 h-12 text-md font-semibold transition-colors flex items-center justify-center border-2 border-[#4a2c2a] ${seatsNeeded === num ? "bg-[#d9573c] text-white" : "bg-[#f4f1de] hover:bg-[#a19b8744]"}`}
                      >
                        {num}
                      </button>
                    ))}
          ...
          <div className="p-6 h-full">
            <div className="text-center mb-8">
              <img
                ...
                alt="User"
                className="w-24 h-24 rounded-full object-cover mx-auto mb-4 border-4 border-[#4a2c2a] shadow-lg"
              />
              <h2 className="text-2xl font-bold">{t.profile}</h2>
              <p className="text-sm">{t.memberSince}</p>
            </div>

            <div className="bg-[#fffbeb] border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a]">
              <button className="w-full text-left p-4 flex items-center hover:bg-[#a19b8744] transition-colors">
                <Car size={22} className="mr-4" />
                <span className="flex-grow font-semibold">{t.goToDriverAccount}</span>
                <ChevronRight size={20} className="text-[#a19b87]" />
              </button>

              <button
                onClick={() => setShowLanguageModal(true)}
                className="w-full text-left p-4 flex items-center border-t-2 border-[#4a2c2a] hover:bg-[#a19b8744] transition-colors"
              >
                <Globe size={22} className="mr-4" />
                <span className="flex-grow font-semibold">{t.language}</span>
              ...
              <a
                ...
                className="w-full text-left p-4 flex items-center border-t-2 border-[#4a2c2a] hover:bg-[#a19b8744] transition-colors"
              >
                <MessageCircle size={22} className="mr-4" />
                <span className="flex-grow font-semibold">{t.support}</span>
                <ChevronRight size={20} className="text-[#a19b87]" />
              </a>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#f4f1de] font-['Press_Start_2P',_monospace] text-[#4a2c2a] antialiased text-xs sm:text-base">
      <CustomScrollbarStyles />

      {!selectedRide && (
        <header className="flex-shrink-0 bg-[#fffbeb] p-4 flex items-center justify-between border-b-2 border-[#4a2c2a] z-20">
          <span className="font-semibold tracking-tighter text-sm sm:text-base">+998 90 123 45 67</span>
          <button
            onClick={() => setActiveTab(activeTab === "profile" ? "search" : "profile")}
            className="p-2 border-2 border-[#4a2c2a] hover:bg-[#a19b8744]"
          >
            {activeTab === "profile" ? <X size={24} /> : <User size={24} />}
          </button>
        </header>
      )}
      ...
      {!selectedRide && !showSearchResults && activeTab === "search" && (
        <div className="fixed bottom-20 left-0 right-0 p-4 bg-transparent z-40">
          <button
            className={`w-full py-3 font-semibold transition-all border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a] hover:shadow-none hover:translate-x-1 hover:translate-y-1 ${
              pickupLocation && destinationLocation && pickupDate && seatsNeeded
                ? "bg-[#d9573c] text-white"
                : "bg-[#a19b87] text-[#f4f1de] cursor-not-allowed"
            }`}
      ...
      {!selectedRide && (
        <nav className="fixed bottom-0 left-0 right-0 bg-[#fffbeb] p-2.5 z-50 border-t-2 border-[#4a2c2a]">
          <div className="flex items-center p-1 max-w-sm mx-auto space-x-2">
            <button
              className={`flex-1 py-2 text-xs font-semibold flex items-center justify-center space-x-2 transition-all border-2 border-[#4a2c2a] ${activeTab === "search" ? "bg-[#4a2c2a] text-[#f4f1de] shadow-inner" : "hover:bg-[#a19b8744]"}`}
              onClick={() => {
                setActiveTab("search");
                setShowSearchResults(false);
                setPickupLocation("");
            ...
            <button
              className={`flex-1 py-2 text-xs font-semibold flex items-center justify-center space-x-2 transition-all border-2 border-[#4a2c2a] ${activeTab === "history" ? "bg-[#4a2c2a] text-[#f4f1de] shadow-inner" : "hover:bg-[#a19b8744]"}`}
...
const AccordionItem = ({ icon, title, value, children }) => {
  ...
  return (
    <div className="border-b-2 last:border-b-0 border-[#4a2c2a]">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="p-4 flex justify-between items-center w-full hover:bg-[#a19b8744]"
      >
        <div className="flex items-center space-x-3">
          {icon}
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm truncate max-w-[120px]">{value}</span>
          <ChevronDown size={20} className={`transition-transform ${isOpen ? "rotate-180" : ""}`} />
        </div>
      </button>
      <div className={`overflow-hidden transition-all duration-300 ${isOpen ? "max-h-96" : "max-h-0"}`}>
        <div className="p-4 bg-[#f4f1de] text-sm">{children}</div>
      </div>
    </div>
  );
};
...
const SpecialService = ({ service }) => {
  ...
  return (
    <div className="flex items-center">
      {iconMap[service] || <Star size={16} className="mr-2" />} {service}
    </div>
  );
};
...
const TripDetails = ({ ride, isUnreliable, onToggleReliability, onBack, onBook }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#f4f1de] font-['Press_Start_2P',_monospace] text-[#4a2c2a] text-xs sm:text-base">
      <div className="bg-[#fffbeb] p-4 flex-shrink-0 border-b-2 border-[#4a2c2a]">
        <div className="flex items-center">
          <button onClick={onBack} className="mr-3 border-2 border-[#4a2c2a] p-1 hover:bg-[#a19b8744]">
            <ChevronLeft size={24} />
          </button>
          <img
            src={ride.imageUrl}
            alt={ride.carModel}
            className="w-24 h-16 object-cover border-2 border-[#4a2c2a] mr-4"
            onError={(e) => (e.target.src = "https://placehold.co/200x150/fffbeb/4a2c2a?text=No+Img")}
          />
          <div className="flex items-stretch w-full">
            <div className="relative flex flex-col justify-between items-center mr-4 shrink-0">
              <div className="absolute top-2.5 bottom-2.5 left-1/2 -translate-x-1/2 w-0.5 bg-[#a19b87]"></div>
              <MapPin size={20} className="text-[#1b998b] bg-[#fffbeb] z-10" />
              <Target size={20} className="text-[#d9573c] bg-[#fffbeb] z-10" />
            </div>
      ...
      <div className="flex-grow overflow-y-auto">
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between text-sm p-2 bg-[#fffbeb] border-2 border-[#d9573c]">
            <span className="text-[#d9573c]">Simulate Unreliable Rider:</span>
            <label className="relative inline-flex items-center cursor-pointer">
              <input type="checkbox" checked={isUnreliable} onChange={onToggleReliability} className="sr-only peer" />
              <div className="w-11 h-6 bg-[#a19b87] peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:h-5 after:w-5 after:transition-all peer-checked:bg-red-600"></div>
            </label>
          </div>

          <h2 className="text-lg font-semibold">About the trip</h2>
          <div className="bg-[#fffbeb] border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a]">
            <AccordionItem icon={<DollarSign />} title="Payout (est.)" value={`$${finalPrice.toFixed(2)}`}>
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <span className={isUnreliable ? "line-through text-[#a19b87]" : ""}>Reliability Discount:</span>
                  <span className={isUnreliable ? "line-through text-[#a19b87]" : "text-[#1b998b]"}>
                    -${discount.toFixed(2)}
                  </span>
                </div>
                {isUnreliable && (
                  <div className="text-xs text-[#d9573c] bg-[#f4f1de] p-2 border border-[#d9573c] flex items-start space-x-2">
                    <Info size={14} className="mt-0.5 flex-shrink-0" />
                    <span>Discount removed due to no-show.</span>
                  </div>
                )}
                <hr className="my-2 border-[#a19b87]" />
                <div className="flex justify-between font-bold">
                  <span>Final Price:</span>
                  <span>${finalPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-xs pt-2">
                  <span>Avg. Fuel/Mile:</span>
                  <span>{ride.avgFuelPerMile}</span>
                </div>
              </div>
            </AccordionItem>
            <AccordionItem icon={<Users />} title="Special services" value="">
              {ride.specialServices.map((service) => (
                <SpecialService key={service} service={service} />
              ))}
            </AccordionItem>
            <AccordionItem icon={<User />} title="Driver & Car" value="">
              <div className="flex space-x-4">
                <img
                  src={ride.driverImageUrl}
                  alt={ride.driverName}
                  className="w-20 h-20 object-cover rounded-full border-2 border-[#4a2c2a]"
                  onError={(e) => (e.target.src = "https://placehold.co/100x100/fffbeb/4a2c2a?text=N/A")}
                />
                <div>
                  ...
                  <div className="flex items-center text-sm">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        key={i}
                        size={14}
                        fill={i < Math.floor(ride.reliabilityStars) ? "#d9573c" : "none"}
                        stroke="#d9573c"
                        className="mr-0.5"
                      />
                    ))}
      ...
      <div className="p-4 bg-[#fffbeb] border-t-2 border-[#4a2c2a] flex-shrink-0 flex justify-between items-center">
        <div>
          <p className="text-2xl font-bold">${finalPrice.toFixed(2)}</p>
          <p className="text-sm">{ride.ratePerMail || ride.avgFuelPerMile}</p>
        </div>
        <button
          onClick={() => onBook(ride)}
          className="bg-[#d9573c] text-[#f4f1de] px-10 py-4 text-lg font-semibold border-2 border-[#4a2c2a] shadow-[4px_4px_0px_#4a2c2a] hover:shadow-none hover:translate-x-1 hover:translate-y-1 transition-all"
        >
          Book
        </button>
      </div>
